...
public abstract class FailbackRegistry extends AbstractRegistry {
  ...
  public String getData(String path){
    try {
      ...
      e.printStackTrace();
      logger.error("读取节点数据值失败:path=" + path);
    }
    return null;
  }
  ...
  protected void recover() throws Exception {
    ...
    if (!recoverRegistered.isEmpty()) {
        logger.info("恢复注册url " + recoverRegistered);
    ...
    if (!recoverSubscribed.isEmpty()) {
        logger.info("恢复订阅url " + recoverSubscribed.keySet());
  ...
  protected void retry() {
    ...
    if (retryCounter++ % 120 == 0) {
      logger.debug("retry method is invoked.");
    }
    ...
    if (!failedSubscribed.isEmpty()) {
      ...
      if (failed.size() > 0) {
          logger.info("重新订阅 " + failed.keySet());
        try {
